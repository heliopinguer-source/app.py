...
# --- KEEP ALIVE / PING STREAMLIT CLOUD ---
            ...
            if nome and zap_cli and defe:
                ...
                try:
                    ...
                    if res.status_code in [200, 201]:
                        st.success(f"✅ Protocolo **{prot}** registrado com sucesso!")
                        ...
                        link_direto = (
                            ...
                            f"phone={WHATS_RECEPCAO}&text={urllib.parse.quote(texto_zap)}"
                        )

                        with col_btn2:
                            st.markdown(f"""
                                <a href="{link_direto}" target="_blank" style="text-decoration:none;">
                                    <div style="background-color:#25D366; color:white; padding:12px;
                                        border-radius:5px; text-align:center; font-weight:bold;
                                        font-size:16px; height:50px; display:flex;
                        ...
                        st.warning("⚠️ Clique no botão verde acima para concluir o envio!")
                    else:
                        st.error("Erro ao salvar os dados.")
                except:
                    st.error("Erro de conexão com o servidor.")
            else:
                st.warning("⚠️ Preencha Nome, WhatsApp e Defeito.")
...
# --- 6. ADMIN ---
    ...
    try:
        ...
        if not df.empty:
            ...
            excluir = st.selectbox("Selecione o Protocolo:", lista_prot)
